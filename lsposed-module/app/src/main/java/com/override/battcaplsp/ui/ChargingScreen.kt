...
fun ChargingScreen(repo: ChgParamRepository, mgr: ChgModuleManager) {
        ...
        AlertDialog(
            ...
            text = {
                Column(
                    ...
                    Text("本功能为实验性功能，通过内核模块直接修改充电参数。", style = MaterialTheme.typography.bodyMedium)
    ...
    Column(
        ...
        AppCard {
            SectionHeader(title = "USB 输入参数 (PPS 功率控制)")
            Spacer(Modifier.height(AppDimensions.SpaceSmall))
            Row(verticalAlignment = Alignment.CenterVertically) {
                OutlinedTextField(
                    icl, { icl = it },
                    ...
                    ExposedDropdownMenuBox(
                        ...
                        ExposedDropdownMenu(
                            ...
                            protocolOptions.forEach { protocol ->
                                DropdownMenuItem(
                                    text = { Text(protocol) },
                                    onClick = { selectedProtocol = protocol; expanded = false }
                                )
                            }
                        }
                    }
                    ...
                    onClick = {
                        scope.launch {
                            try {
                                ...
                                if (!vMaxValid) { msg = "ERROR:${vMaxErr}"; OpEvents.error("充电:${vMaxErr}"); return@launch }
                                ...
                                if (!cccValid) { msg = "ERROR:${cccErr}"; OpEvents.error("充电:${cccErr}"); return@launch }
                                ...
                                if (!termValid) { msg = "ERROR:${termErr}"; OpEvents.error("充电:${termErr}"); return@launch }
                                ...
                                if (!iclValid) { msg = "ERROR:${iclErr}"; OpEvents.error("充电:${iclErr}"); return@launch }
                                ...
                                if (!ivlValid) { msg = "ERROR:${ivlErr}"; OpEvents.error("充电:${ivlErr}"); return@launch }
                                ...
                                if (!limitValid) { msg = "ERROR:${limitErr}"; OpEvents.error("充电:${limitErr}"); return@launch }
