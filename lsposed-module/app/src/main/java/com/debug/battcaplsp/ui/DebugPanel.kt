...
fun DebugPanel(moduleManager: ModuleManager) {
    ...
    Column(
        ...
        AppCard {
            SectionHeader(title = "Root 状态")
            Spacer(Modifier.height(AppDimensions.SpaceSmall))
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(rootStatus, style = MaterialTheme.typography.bodyLarge, modifier = Modifier.weight(1f))
                ActionButton(
                    text = "刷新 Root",
                    onClick = {
                        scope.launch {
        ...
        CollapsibleCard(title = "内核模块参数 Dump") {
            ButtonRow {
                ActionButton(
                    text = "读取",
                    onClick = {
        ...
        CollapsibleCard(title = "临时内核模块测试", description = "1. 从给定 URL 下载 .ko 到应用内部目录  2. 手动 insmod  3. 抓取最近 dmesg") {
            ...
            ButtonRow {
                ActionButton(
                    ...
                    onClick = {
                        ...
                        if (url.isEmpty()) { scope.launch { snackbarHostState.showSnackbar("URL 不能为空") }; return@ActionButton }
                        scope.launch {
                    ...
                    DropdownMenu(expanded = showPreset, onDismissRequest = { showPreset = false }) {
                        if (loadingKoList) {
                            ...
                            koAssets.forEach { asset ->
                                DropdownMenuItem(
                                    text = { Text("${asset.name} (${asset.tag})", maxLines = 1, overflow = TextOverflow.Ellipsis) },
                                    onClick = {
                ...
                ActionButton(text = "刷新列表", onClick = { tempListRefreshToggle = !tempListRefreshToggle }, enabled = downloaded.isNotEmpty() && !loading, secondary = true)
                ActionButton(
                    text = "清理全部",
                    onClick = {
            ...
            if (tempDownloadMsg.isNotBlank()) {
                ...
                Text(
                    ...
                    color = when {
                        tempDownloadMsg.startsWith("SUCCESS") -> MaterialTheme.colorScheme.primary
                        tempDownloadMsg.startsWith("ERROR") -> MaterialTheme.colorScheme.error
            ...
            if (tempLoadMsg.isNotBlank()) {
                ...
                Text(
                    tempLoadMsg,
                    color = when {
                        tempLoadMsg.startsWith("SUCCESS") -> MaterialTheme.colorScheme.primary
                        tempLoadMsg.startsWith("ERROR") -> MaterialTheme.colorScheme.error
                        else -> MaterialTheme.colorScheme.secondary
                    }
                )
            }
    ...
    AppCard {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                SectionHeader(title = title, description = if (expanded) null else description)
            }
